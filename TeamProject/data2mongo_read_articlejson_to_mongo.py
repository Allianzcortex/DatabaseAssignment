        ...
        for line in input_:
            ...
            try:
                ...
                if(isinstance(name_str,list)):
                    name_str=name_str[0]
                authors=([values.encode('ascii','ignore') for values in name_str.values() if values.encode('ascii','ignore').strip()!=""])
            except KeyError:
                authors=" "
            try:
                    if("ftext" in s["title"]):
            ...
            try:
                ...
                journal=""
            try:
                if(title=="except_count"):
                    except_count+=1
                article_item = {
                "_id":_doc_index,
                "title":title,
                "journal":journal,
                "volume":s["volume"]["ftext"].encode('ascii','ignore'),
                "year":s["year"]["ftext"].encode('ascii','ignore'),
                "pages":pages,
                "authors":authors
                }
